...
async fn main() -> Result<()> {
    ...
    let wasi_ctx = WasiCtxBuilder::new()
        ...
        .inherit_env()
        .build_p1();
    // создать хранилище в памяти и экземпляр модуля WASM
    ...
    // выделить память для входных данных
    ...
    // получаем точку входа функцию из модуля WASM
    let entry_point = instance.get_typed_func::<(i32, i32), i32>(
        &mut store, "entry_point"
    ).unwrap();
    // вызываем функцию точки входа, чтобы получить указатель на результат
    let ret = entry_point.call_async(
        &mut store, (input_data_ptr, serialized.len() as i32)
    ).await.unwrap();
    // получить указатель результата из памяти
