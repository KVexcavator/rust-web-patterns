...
pub fn get_all<T: DeserializeOwned>() -> Result<HashMap<String, T>, String> {
  ...
  let tasks: HashMap<String, T> = serde_json::from_str(&contents).map_err(|e| format!("Error parsing JSON: {}", e))?;
...
pub fn get_one<T: DeserializeOwned + Clone>(id: &str) -> Result<T, String> {
  ...
  match tasks.get(id) {
    Some(t) => Ok(t.clone()),
    None => Err(format!("Task with id {} not find", id))
  }
}
